<template>
  <div>
    ...
    <el-table :data="orders" border style="width: 100%">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button @click="trueClick(scope.row)" type="text" size="small" :class="scope.row.status=='完成交易'?'hide':''">确认完成</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog title="订单详情" :visible.sync="dialogVisible" width="300px"  class="diaCenter" v-if="details">
      ...
      <div><span class="rig">商品：</span><span>{{details.commodities.name}}</span></div>
      <div><span class="rig">净重：</span><span>{{details.commodities.weight}}</span></div>
      <div><span class="rig">建议零售价：</span><span>{{details.commodities.price}}</span></div>
      <div><span class="rig">产地：</span><span>{{details.commodities.palce}}</span></div>
      <div><span class="rig">保质期：</span><span>{{details.commodities.shelfLife}}</span></div>
      <div><span class="rig">味道：</span><span>{{details.commodities.taste}}</span></div>
      <div><span class="rig">特殊功用：</span><span>{{details.commodities.specialFunc}}</span></div>
      <div><span class="rig">供应商：</span><span>{{details.commodities.supplier}}</span></div>
